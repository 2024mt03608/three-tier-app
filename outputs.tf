# Outputs the URL of the deployed frontend Cloud Run service
output "endpoint" {
  description = "The URL of the deployed frontend (Cloud Run service 'fe')"
  value       = google_cloud_run_service.fe.status[0].url
}

# Outputs the name of the Cloud SQL instance created
output "sql_instance_name" {
  description = "The name of the Cloud SQL instance created"
  value       = google_sql_database_instance.main.name
}

# Outputs the resource name of the DB password secret in Secret Manager (only if MySQL is used)
...
# Provides a URL to the GCP Cloud Console walkthrough for deploying this app
